...
def get_options_chain(ticker="TSLA"):
    try:
        tk = yf.Ticker(ticker)
        expirations = tk.options[:3]  # first 3 expirations
        chains = []
...
with gr.Blocks(title="XForge Trader", theme=gr.themes.Soft()) as app:
    ...
    gr.Markdown("**Live • Charts • Options • Self-Improvement**")

    with gr.Tabs():
        # Dashboard
        with gr.Tab("📊 Live Dashboard"):
            gr.Markdown("### Real-time Prices & Interactive Chart")
            with gr.Row():
        ...
        with gr.Tab("📋 Options Chain"):
            ...
            options_output = gr.Markdown(label="Options Data")
        ...
        with gr.Tab("📥 Historical Data"):
            ...
            backfill_btn = gr.Button("Backfill Historical Data")
            backfill_output = gr.Textbox()
            backfill_btn.click(lambda x: db.backfill([t.strip().upper() for t in x.split(",")]), 
                             inputs=tickers_input, outputs=backfill_output)

        # Self-Improvement
        with gr.Tab("🧠 Self-Improvement"):
            prompt = gr.Textbox(lines=4, placeholder="Make the candlestick chart update every 10 seconds...")
            sim_btn = gr.Button("Trigger SIM Cycle")
            sim_output = gr.Textbox()
            sim_btn.click(lambda p: f"✅ Self-improvement cycle started with prompt:\n{p}\n\n(Next iteration: xAI integration)", 
                         inputs=prompt, outputs=sim_output)
    ...
    gr.Markdown("---\n**Status:** Live updates running • Auto browser open enabled")

# ========================= LAUNCH =========================
if __name__ == "__main__":
    logger.info("🚀 Starting XForge Trader with Charts & Options...")
    print("🌐 Opening XForge Trader in your browser...")
    app.launch(
