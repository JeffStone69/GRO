# ============================================================
# xforge_trader.py  —  xForgeTrader V11
# Complete Self-Improving Profit Recommendation Engine
# Modular | Robust | Grok-Powered | Never Fails
# Includes: Alpha Vantage, Ensemble Scanner, Portfolio Optimizer
# ============================================================
...
# ============================================================
# MODULAR SECTION 4: DATA LAYER (Demo + yfinance + Alpha Vantage + Fallbacks)
# ============================================================
...
def get_alpha_vantage_data(ticker, api_key, start_date=None, end_date=None):
    if not api_key:
        return None
    try:
...
def get_data(ticker, start_date=None, end_date=None, realtime=False, demo_mode=True, alpha_key=""):
    ticker = ticker.strip().upper()
    if demo_mode:
        return get_demo_data(ticker)

    # Try Alpha Vantage first (new v11 feature)
    if alpha_key:
        av_df = get_alpha_vantage_data(ticker, alpha_key, start_date, end_date)
...
# ============================================================
# MODULAR SECTION 7: ENSEMBLE SCANNER (V11 Enhanced)
# ============================================================
def scan_tickers(tickers_str, capital, risk_pct, start_date, end_date, realtime, demo_mode, alpha_key):
    tickers = [t.strip().upper() for t in tickers_str.split(',') if t.strip()]
    results = []
    top_chart = None
    top_rec = None
    ...
    for ticker in tickers[:12]:
        try:
            df_raw = get_data(ticker, start_date, end_date, realtime, demo_mode, alpha_key)
            if df_raw.empty:
                continue
            ...
            # Ensemble probability (V11 new)
            rebound_signal = 1 if (df['Close'].iloc[-1] / df['Close'].cummax().iloc[-1] - 1) <= -0.07 else 0
            ma_signal = 1 if latest.get('SMA50', 0) > latest.get('SMA200', 0) else 0
            rsi_signal = 1 if rsi < 58 else 0
            ensemble_prob = (rebound_signal + ma_signal + rsi_signal) / 3
            prob = max(0.52, min(0.82, 0.58 + ensemble_prob * 0.24 + (50 - rsi) / 200))
    ...
    if not results:
        ...
        return pd.DataFrame([{"Ticker": "DEMO", "Signal": "DEMO MODE", "Price": 0, "Stop Loss": 0, "Target": 0, "Position %": 0, "Ensemble Prob": "0%", "Edge": 0, "RSI": 0}]), create_candlestick(demo_df, "TSLA"), "Demo data loaded"
...
Recent errors in xForgeTrader V11:
{errors}
User note: {user_note}

Provide specific, actionable code changes, refactors, or new features (especially ensemble/ML upgrades). Return concrete diffs or new functions."""
    try:
        from openai import OpenAI
        client = OpenAI(api_key=api_key, base_url="https://api.x.ai/v1")
...
# ============================================================
# MODULAR SECTION 9: PORTFOLIO OPTIMIZER (NEW V11 TAB)
# ============================================================
def optimize_portfolio(top_picks_df, total_capital):
    if top_picks_df.empty or "Edge" not in top_picks_df.columns:
        return "No BUY signals to optimize"
    buys = top_picks_df[top_picks_df["Signal"] == "BUY"].head(5)
    if buys.empty:
        return "No BUY recommendations found"
    
    total_edge = buys["Edge"].sum()
    allocations = []
    for _, row in buys.iterrows():
        weight = row["Edge"] / total_edge if total_edge > 0 else 0.2
        alloc = round(weight * total_capital, 2)
        allocations.append({
            "Ticker": row["Ticker"],
...
# ============================================================
# MODULAR SECTION 10: FULL GRADIO UI (V11)
# ============================================================
with gr.Blocks(title="xForgeTrader V11", theme=gr.themes.Soft()) as app:
    gr.Markdown("# xForgeTrader V11 — Profit Recommendation Engine")
    gr.Markdown("**Never fails • Self-improving • Grok-powered • Alpha Vantage + Ensemble + Portfolio**")
    ...
    with gr.Tab("Scanner"):
        ...
        with gr.Row():
            ...
            alpha_key = gr.Textbox("", label="Alpha Vantage API Key (optional for live data)", type="password")
        scan_btn = gr.Button("Scan & Recommend (Ensemble)", variant="primary")
        ...
        scan_btn.click(
            fn=scan_tickers,
            inputs=[tickers, capital, risk_pct, start, end, realtime, demo, alpha_key],
    ...
    with gr.Tab("Portfolio Optimizer (NEW V11)"):
        port_df = gr.Dataframe(label="Top Picks from Scanner")
        port_capital = gr.Number(25000, label="Total Capital to Allocate ($)")
        port_btn = gr.Button("Optimize Portfolio Allocation")
        port_out = gr.Dataframe(label="Recommended Allocations")
        port_btn.click(
            fn=optimize_portfolio,
            inputs=[port_df, port_capital],
            outputs=port_out
        )
    ...
    gr.Markdown("V11 • Modular • Error-driven self-improvement • Alpha Vantage + Ensemble + Portfolio • Demo always works")
