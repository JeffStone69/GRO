...
with gr.Blocks(title="xForgeTrader v8", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# 🧠 xForgeTrader v8 – Demo Mode ON by Default + macOS Fixed\n**Upload this v8 to your GitHub FORGE folder**")
    ...
    with gr.Tab("🚪 EXIT"):
        ...
        gr.Markdown("""
        1. Create `Run-xForgeTrader.command` in the FORGE folder (use the content I gave you).
        2. Run `chmod +x Run-xForgeTrader.command` in Terminal.
        3. Right-click the `.command` file → **Open** (this bypasses macOS blocking).
        """)

    gr.Markdown("v8 – Demo Mode ON by default • macOS Gatekeeper fix included • Ready for GitHub. Educational only.")
