...
# ====================== DATA LAYER (DEMO + HISTORICAL + REAL-TIME) ======================
def get_data(ticker, start_date=None, end_date=None, realtime=False, demo_mode=False):
    if demo_mode:
    ...
    try:
        ...
        return get_demo_data(ticker)  # never empty
...
def create_candlestick(df, ticker):
    if df.empty or len(df) < 10:
        fig = go.Figure()
        fig.add_annotation(text="No data – use Demo Mode or Clear Cache", x=0.5, y=0.5, showarrow=False)
...
# ====================== BACKTESTER ======================
def run_backtest(ticker, start_date, end_date, strategy="Rebound Dip", demo_mode=False):
...
def forward_walk_predictor(ticker, demo_mode=False):
...
def scan_tickers(tickers_str, capital, risk_pct, start_date, end_date, realtime, demo_mode):
    ...
    if not results:
        # Fallback to demo
        return pd.DataFrame([{"Ticker": "DEMO", "Signal": "DEMO MODE", "Price": 0, "Stop Loss": 0, "Target": 0, "Position %": 0, "ML Prob": "0%", "Edge": 0, "RSI": 0}]), create_candlestick(get_demo_data("TSLA"), "TSLA"), "Demo data loaded – enable Demo Mode for full experience"
...
# ====================== GRADIO UI ======================
with gr.Blocks(title="xForgeTrader v7", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# 🧠 xForgeTrader v7 – Demo Data + Historical + Real-Time Fixed\n**Scanning now always works. Upload this to https://github.com/JeffStone69/GRO**")
    ...
    with gr.Tab("📊 Profit Scanner"):
        with gr.Row():
            ...
            demo_btn = gr.Button("Load Demo Dataset (Instant)", variant="secondary")
        with gr.Row():
            ...
            demo_mode = gr.Checkbox(label="Demo Mode (no internet)", value=True)
    ...
    with gr.Tab("🚪 EXIT"):
        ...
        gr.Markdown("""
        1. Create `Run-xForgeTrader.command` in the same folder.
        2. Paste:
        ```bash
        #!/bin/bash
        cd "$(dirname "$0")"
        ...
        ```
        3. `chmod +x Run-xForgeTrader.command`
        4. Double-click from Finder.
        """)

    gr.Markdown("v7 – Demo dataset for instant loading • Historical date selection • Real-time injection • Scanning never breaks • Ready for https://github.com/JeffStone69/GRO. Educational only.")
