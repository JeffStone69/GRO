...
def log_grok_improvement(suggestion: str):
    """Logs Grok improvement to DB and appends to history"""
    conn = sqlite3.connect(DB_PATH)
...
def scan_tickers(tickers_str, capital, risk_pct, start_date, end_date, realtime, demo_mode):
    ...
    if not results:
        ...
        return pd.DataFrame([{"Ticker": "DEMO", "Signal": "DEMO MODE", "Price": 0, "Stop Loss": 0, "Target": 0, "Position %": 0, "ML Prob": "0%", "Edge": 0, "RSI": 0}]), create_candlestick(demo_df, "TSLA"), "Demo data loaded – turn Demo Mode ON for full experience"
...
# ====================== GRADIO UI (FULLY RESTORED) ======================
with gr.Blocks(title="xForgeTrader – Restored Tabs + Auto History & Self-Improve", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# xForgeTrader – Fully Restored Tabs + Auto History Updates\n**Demo Mode recommended for instant testing**")
    ...
    gr.Markdown("Restored full tabs • Auto HISTORY.md updates • DB-driven self-improvement • Educational use only.")
