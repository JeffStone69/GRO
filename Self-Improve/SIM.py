#!/usr/bin/env python3
"""
XForge Self-Improvement Module - Full Production Version
With GitHub support, file creation, export, logging, and iteration.
"""
...
class SelfImproveConfig(BaseModel):
    ...
    config_file: str = "xforge_config.json"
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            if "grok_api_key" in data and data["grok_api_key"].strip():
                CONFIG.grok_api_key = SecretStr(data["grok_api_key"])
        except Exception:
            pass

def save_config() -> None:
    try:
        data = {"grok_model": CONFIG.grok_model, "grok_api_key": CONFIG.grok_api_key.get_secret_value()}
...
def build_ui():
    ...
    """
    with gr.Blocks(title="XForge Self-Improvement") as demo:
        splash = gr.HTML(value=splash_html, visible=True)
        
        with gr.Row(visible=False) as api_key_section:
            with gr.Column():
                gr.Markdown("### 🔑 Grok API Key")
                api_key_input = gr.Textbox(label="xAI Grok API Key", type="password")
...
if __name__ == "__main__":
    print("\n🚀 Starting XForge Self-Improvement...")
    app = build_ui()
