#!/usr/bin/env python3
"""
XForge Self-Improvement Module - Secure Production Version (FIXED)
...
- Full functionality preserved: GitHub analysis, error logging, CSV export, improved code saving.
"""
...
def self_improve(script_content: str = "", github_url: str = "", user_feedback: str = "") -> tuple[str, str]:
    # Priority: CONFIG (set via UI or env) > environment variable
...
def build_ui():
    ...
    <div style="text-align:center;padding:80px;background:linear-gradient(135deg,#0f172a,#1e2937);color:white;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;">
        ...
        <h2 style="color:#22c55e;">Self-Improvement Module (Secure)</h2>
    ...
    with gr.Blocks(title="XForge Self-Improvement (Secure)", theme=gr.themes.Base()) as demo:
        splash = gr.HTML(value=splash_html, visible=True)

        with gr.Row(visible=False) as api_key_section:
            with gr.Column():
                gr.Markdown("### 🔒 Grok API Key (Memory-Only – Never Saved)")
                gr.Markdown("**Best practice:** Set `XAI_API_KEY` in your terminal before running. If not set, enter it here once.")
        ...
        with gr.Row(visible=False) as main_section:
            ...
            with gr.Tabs():
                with gr.Tab("Analyze & Iterate"):
                    ...
                    feedback_input = gr.Textbox(label="Iteration Instructions / Feedback (optional)", lines=3)
        ...
        def init_app():
            ...
            return (
                gr.update(visible=False),           # hide splash
                gr.update(visible=not has_key),     # show key prompt only if needed
                gr.update(visible=has_key)          # show main UI if key present
            )

        demo.load(init_app, outputs=[splash, api_key_section, main_section])

        def save_handler(key: str):
            success, msg = validate_grok_key(key)
            if success:
                CONFIG.grok_api_key = SecretStr(key.strip())
                # Key is now in memory only for this session. Never written to disk.
        ...
        def analyze(script_text, github_url, uploaded_file, feedback):
            ...
            if uploaded_file is not None:
                try:
                    ...
                    content = "Failed to read uploaded file."
